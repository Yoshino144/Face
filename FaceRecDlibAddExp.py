...
def recongition():
    ...
    while True:
        ...
        for i, d in enumerate(dets):
            ...
            if probability > 0.50:
                cv2.putText(frame, name, (x2 + 0, y1 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            else:
                cv2.putText(frame, 'unknow', (x2 + 0, y1 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            emotion_text = exp.exp(frame_gray[x1-10:y1+10, x2-10:y2+10])
            cv2.putText(frame, emotion_text, (x2 + 0, y1 + 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
